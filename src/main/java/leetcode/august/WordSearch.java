...
public class WordSearch {
  ...
  public boolean exist(char[][] board, String word) {
    ...
    for (int i = 0; i < board.length && !answer; i++) {
      for (int j = 0; j < board[i].length && !answer; j++) {
        // find starting point
        if (board[i][j] == str[0]) {

          index = dfs(board, i, j, index, backtrack, str);
  ...
  public int dfs(char[][] board, int i, int j, int index, String backtrack, char[] str) {
    ...
    index = dfs(board, i - 1, j, index + 1, backtrack, str);
    index = dfs(board, i, j + 1, index + 1, backtrack, str);
    index = dfs(board, i + 1, j, index + 1, backtrack, str);
    index = dfs(board, i, j - 1, index + 1, backtrack, str);
